...
def train(model, best_result, best_epoch, srcc_dict):
    ...
    all_result = {'val':{}, 'test':{}}
    if (epoch >= 0):
        srcc = eval(test_loaders, phase='val', dataset='koniq10k')
...
def eval(test_loaders, phase, dataset):
    model.eval()
    q_mos = []
    q_hat = []
    loaders = []
...
for session in range(0,1):
    ...
    spaq_loader = set_dataset(spaq_meta, 32, img_dir, num_workers, preprocess3,
                                              train_patch, False, set=0)
    liveitw_loader = set_dataset(liveitw_meta, 32, img_dir, num_workers, preprocess3,
                                              train_patch, False, set=0)
    livefb_loader = set_dataset(livefb_meta, 32, img_dir, num_workers, preprocess3,
                                              train_patch, False, set=0)
    agiqa_loader = set_dataset(agiqa_meta, 32, img_dir, num_workers, preprocess3,
                                              train_patch, False, set=0)
